...
% [t] sets the frame alignment to the top, alternatves: c (center), b (bottom) -- can also be defined in the document class options 
	...
	Otherwise \dots
		...
		\begin{itemize}
			\item some \gqq{Style} elements are stored in \gray{tex-styleHU/huberlin.sty} 
			\item packages are stored in \gray{01packages-slides.tex}
			\item customised commands are stored in \gray{01commands-slides.tex} 
			\begin{itemize}
				\item those should be loaded into the main .tex file via  \texttt{\textbackslash input\{\}} before \texttt{\textbackslash begin\{document\}} 
			\end{itemize}
		\end{itemize}
...
\begin{frame}{Introduction}
	...
	\begin{itemize}
		...
		\item[] thus information and elements should be used sparsely to not distract the audience from what you have to say. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Useful Presenation Elements}
\frame{
	%\begin{multicols}{2}
	%\frametitle{~}
	\tableofcontents[currentsection,hidesubsections]
